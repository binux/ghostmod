...
static int _client_motdw3(t_connection * c, t_packet const *const packet)
{
    ...
    sprintf(serverinfo, "欢迎来到 " PVPGN_SOFTWARE " 当前版本 " PVPGN_VERSION "\r\n\r\n目前有 %u 名用户在 %u 个游戏中进行 %s, 同时在%s服务器上有 %u 名用户在进行 %u 个游戏或在 %u 个频道中聊天 .\r\n%s", conn_get_user_count_by_clienttag(conn_get_clienttag(c)), game_get_count_by_clienttag(ctag), clienttag_get_title(conn_get_clienttag(c)), prefs_get_servername(),connlist_login_get_length(), gamelist_get_length(), channellist_get_length(), prefs_get_server_info());
...
static int _glist_cb(t_game * game, void *data)
{
	...
	char game_name_temp[MAX_MESSAGE_LEN];
    sprintf(game_name_temp, "%s +%d", game_get_name(game), game_get_ref(game));
	if(strlen(game_name_temp)<=31)
		packet_append_string(cbdata->rpacket, game_name_temp);
	else
...
static int _client_gamelistreq(t_connection * c, t_packet const *const packet)
{
	...
	if ((game = gamelist_find_game2(gamename, clienttag, gtype))) {
		...
		case game_status_loaded:
			...
			sprintf(game_name_temp, "%s +%d", game_get_name(game), game_get_ref(game));
			if(strlen(game_name_temp)<=31)
				packet_append_string(rpacket, game_name_temp);
			else
				packet_append_string(rpacket, game_get_name(game));
...
static int _client_joingame(t_connection * c, t_packet const *const packet)
{
	...
	if (!(game = gamelist_find_game2(gamename, conn_get_clienttag(c), game_type_all))) {
