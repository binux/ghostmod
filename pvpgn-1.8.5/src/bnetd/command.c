...
static int _handle_voice_command(t_connection * c, char const * text)
{
    ...
    dst_c = account_get_conn(acc);
    if (account_get_auth_voice(acc,channel)==1)
	snprintf(msgtemp, sizeof(msgtemp), "%s is already on VOP list, no need to Voice him", username);
    else
    {
...
static int _handle_friends_command(t_connection * c, char const * text)
{
	...
	if(flist!=NULL) {
		...
		friend=account_get_name(friend_acc);
    		if (software[0]) snprintf(msgtemp, sizeof(msgtemp), "%d: %c%.16s%.128s, %.64s", i+1, friend_get_mutual(fr)?'*':' ', friend, status,software);
		else snprintf(msgtemp, sizeof(msgtemp), "%d: %.16s%.128s", i+1, friend, status);
		message_send_text(c,message_type_info,c,msgtemp);
	    }
	}
	message_send_text(c,message_type_info,c,"=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=");
...
static int _handle_uptime_command(t_connection * c, char const *text)
{
  ...
  snprintf(msgtemp, sizeof(msgtemp), "Uptime: %.128s",seconds_to_timestr(server_get_uptime()));
  message_send_text(c,message_type_info,c,msgtemp);
...
//	 }
   ...
   {
     snprintf(msgtemp, sizeof(msgtemp), "max channel name length exceeded (max %d symbols)", CHANNEL_NAME_LEN - 1);
     message_send_text(c,message_type_error,c,msgtemp);
     return 0;
   }
...
static int _glist_cb(t_game *game, void *data)
{
    ...
    {
	snprintf(msgtemp, sizeof(msgtemp), " %-16.16s %1.1s %-8.8s %-15.15s %5u %6u ",
	    game_get_name(game),
	    game_get_flag(game) != game_flag_private ? "n":"y",
	    game_status_get_str(game_get_status(game)),
	    game_type_get_str(game_get_type(game)),
	    game_get_ref(game),
            (unsigned int)difftime(now,game_get_start_time(game)==0?game_get_create_time(game):game_get_start_time(game))/60);
...
static int _handle_games_command(t_connection * c, char const *text)
{
  ...
  snprintf(msgtemp, sizeof(msgtemp), " ------name------ p -status- -----type------ count lasted ");
  if (!cbdata.tag)
    strcat(msgtemp,"ctag ");
...
static int _handle_chpass_command(t_connection * c, char const *text)
{
  ...
  {
    snprintf(msgtemp, sizeof(msgtemp), "Maximum password length allowed is %d",USER_PASS_MAX);
    message_send_text(c,message_type_error,c,msgtemp);
    return 0;
  }
  ...
  snprintf(msgtemp, sizeof(msgtemp), "Trying to change password for account \"%.64s\" to \"%.64s\"",username,pass);
  message_send_text(c,message_type_info,c,msgtemp);
...
static int _handle_connections_command(t_connection *c, char const *text)
{
  ...
  LIST_TRAVERSE_CONST(connlist(),curr)
  {
	...
	if (prefs_get_hide_addr() && !(account_get_command_groups(conn_get_account(c)) & command_get_group("/admin-addr"))) /* default to false */
	  ...
	  snprintf(msgtemp, sizeof(msgtemp), " %3u %-6.6s %-12.12s %4.4s %-15.15s 0x%08x 0x%04x %9u %-16.16s %-8.8s %s",
		  conn_get_socket(conn),
		  conn_class_get_str(conn_get_class(conn)),
		  conn_state_get_str(conn_get_state(conn)),
		  tag_uint_to_str(clienttag_str,conn_get_fake_clienttag(conn)),
...
static int _handle_gameinfo_command(t_connection * c, char const *text)
{
  ...
  snprintf(msgtemp, sizeof(msgtemp), "Name: %-20.20s    ID: "GAMEID_FORMAT" (%s)",game_get_name(game),game_get_id(game),game_get_flag(game) != game_flag_private ? "public":"private");
  message_send_text(c,message_type_info,c,msgtemp);
  ...
  snprintf(msgtemp, sizeof(msgtemp), "Status: %.64s",game_status_get_str(game_get_status(game)));
  message_send_text(c,message_type_info,c,msgtemp);

  snprintf(msgtemp, sizeof(msgtemp), "Type: %-20.20s",game_type_get_str(game_get_type(game)));
  message_send_text(c,message_type_info,c,msgtemp);

  snprintf(msgtemp, sizeof(msgtemp), "Speed: %.64s",game_speed_get_str(game_get_speed(game)));
  message_send_text(c,message_type_info,c,msgtemp);

  snprintf(msgtemp, sizeof(msgtemp), "Difficulty: %.64s",game_difficulty_get_str(game_get_difficulty(game)));
  message_send_text(c,message_type_info,c,msgtemp);

  snprintf(msgtemp, sizeof(msgtemp), "Option: %.64s",game_option_get_str(game_get_option(game)));
  message_send_text(c,message_type_info,c,msgtemp);
  ...
  message_send_text(c,message_type_info,c,msgtemp);
  snprintf(msgtemp, sizeof(msgtemp), "Map Tileset: %.64s",game_tileset_get_str(game_get_tileset(game)));
  message_send_text(c,message_type_info,c,msgtemp);
  snprintf(msgtemp, sizeof(msgtemp), "Map Type: %.64s",game_maptype_get_str(game_get_maptype(game)));
  message_send_text(c,message_type_info,c,msgtemp);
  ...
  for (i=0; i<game_get_count(game); i++)
		...
		{
		  snprintf(msgtemp, sizeof(msgtemp), "%-32.32s(IP:%-16.16s) latency: %9u",conn_get_username(user),addr_num_to_ip_str2(conn_get_game_addr(user)),conn_get_latency(user));
		  message_send_text(c,message_type_info,c,msgtemp);
		}
	    }
  {
  ...
  // --> aaron
	...
	/*
	  if (account_get_atteammembers(account,teamcount,clienttag))
	    ...
	    snprintf(msgtemp, sizeof(msgtemp), "WarCraft3 AT Team   %5u: <invalid team info>",rank);
	}
      else
...
static int _handle_lockacct_command(t_connection * c, char const *text)
{
  ...
  if (username[0]=='\0')
    {
      message_send_text(c,message_type_info,c,"usage: /lockacct <username> <time>");
      return 0;
    }
  ...
  if ((user = connlist_find_connection_by_accountname(username))){
    ...
    sprintf(tstr,"%s is locked by %s.for %s",conn_get_username(user),conn_get_username(c),time==0?"permamently":seconds_to_timestr(time-now));
    eventlog(eventlog_level_info,__FUNCTION__,tstr);
  }
  
  time=ipbanlist_str_to_time_t(c,&text[i]);
  account_set_ll_time(account,time);
  account_set_auth_lock(account,1);
  sprintf(tstr,"That user account is now locked.  for %s",time==0?"permamently":seconds_to_timestr(time-now));
...
static int _handle_tag_command(t_connection * c, char const *text)
{
	...
	channel_update_userflags(c);
        snprintf(msgtemp, sizeof(msgtemp), "Client tag set to %.64s.",dest);
    }
    else
    snprintf(msgtemp, sizeof(msgtemp), "Invalid clienttag %.64s specified",dest);
    message_send_text(c,message_type_info,c,msgtemp);
    return 0;
}
...
static int _handle_ping_command(t_connection * c, char const *text)
{
  ...
  if (text[i]=='\0')
    ...
    snprintf(msgtemp, sizeof(msgtemp), "%-32.32s(IP:%-16.16s) latency %9u",&text[i],addr_num_to_ip_str2(conn_get_game_addr(user)),conn_get_latency(user));
  else
...
static int _handle_commandgroups_command(t_connection * c, char const * text)
{
	...
	tempgroups[8] = '\0';
	snprintf(msgtemp, sizeof(msgtemp), "%.64s's command group(s): %.128s", username, tempgroups);
	message_send_text(c,message_type_info,c,msgtemp);
	return 0;
    }
	...
	snprintf(msgtemp, sizeof(msgtemp), "groups %.128s has been added to user: %.64s", arg3, username);
	message_send_text(c,message_type_info,c,msgtemp);
	return 0;
    }
	...
	snprintf(msgtemp, sizeof(msgtemp), "groups %.128s has been deleted from user: %.64s", arg3, username);
	message_send_text(c,message_type_info,c,msgtemp);
	return 0;
    }

    snprintf(msgtemp, sizeof(msgtemp), "got unknown command: %.64s", command);
    message_send_text(c,message_type_info,c,msgtemp);
    return 0;
}
...
static int _handle_topic_command(t_connection * c, char const * text)
{
  ...
  {
    message_send_text(c,message_type_error,c,"no such channel, can't set topic");
    return -1;
  }
