...
extern char const * game_get_name(t_game const * game)
{
	...
	//sprintf(game->game_name_temp, "%s [%d]", game->name ? game->name : "BNet", game_get_count(game) );
	//return game->game_name_temp;
...
int strcasecmp2(char const * str1, char const * str2)
{
    unsigned int i;
    int          a;
    int          b;
...
extern t_game * gamelist_find_game2(char const * name, t_clienttag ctag, t_game_type type)
{
    t_elist *curr;
    t_game *game;
	t_game *result = NULL;
	...
	if ((type==game_type_all || game->type==type) 
	    && ctag == game->clienttag 
	    && game->name 
	    && max < strcasecmp2(name,game->name))
		{
			result = game;
			max = strcasecmp2(name,game->name);
		}
