...
static int _glist_cb_xml(t_game *game, void *data)
{
    ...
    fprintf((FILE*)data,"\t\t<game><name>%s</name><clienttag>%s</clienttag><status>%s</status><count>%d</count><max>%d</max></game>\n",game_get_name(game),tag_uint_to_str(clienttag_str,game_get_clienttag(game)),game_status_get_str(game_get_status(game)),game_get_count(game),game_get_maxplayers(game));
